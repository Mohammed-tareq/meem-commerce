...
class PromotionApplicator
{
    ...
    public function applyOutcome(
        ...
        return DB::transaction(function () use ($cart, $promotion, $outcome) {
            ...
            // compute current subtotal in cents (exclude gift items)
            $subtotalCents = (int) round($cart->items->reject(fn($i) => (bool) ($i->is_gift ?? false))->sum(fn($i) => (float) ($i->total_price ?? 0)) * 100);
